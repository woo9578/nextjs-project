...
export async function POST(req) {
  try {
    ...
    const sql = `SELECT site_cd,str_cd,"KAKAO" AS type,DATE_FORMAT(TRAN_DATE,'%Y-%m-%d %T') AS send_time,concat(substring(tran_phone,1,3) , '****', substring(tran_phone,-4)) AS phone,TRAN_MSG AS talk_msg, case when TRAN_RSLT = '1000' then 'sucess' ELSE 'fail' END AS result 
                FROM mts_atalk_msg_log WHERE site_cd= '${requestBody.site_cd}' AND str_cd = '${requestBody.str_cd}' AND (TRAN_DATE >= '${requestBody.selectDate.st_date} 00:00:00' AND TRAN_DATE <= '${requestBody.selectDate.end_date} 23:59:59') UNION ALL 
                ...
                FROM mts_sms_msg_log WHERE site_cd= '${requestBody.site_cd}' AND str_cd = '${requestBody.str_cd}' AND (TRAN_DATE >= '${requestBody.selectDate.st_date} 00:00:00' AND TRAN_DATE <= '${requestBody.selectDate.end_date} 23:59:59') UNION ALL 
                ...
                FROM tb_agent_message_log WHERE site_cd= '${requestBody.site_cd}' AND str_cd = '${requestBody.str_cd}' AND (REQ_SEND_DATE >= '${requestBody.selectDate.st_date} 00:00:00' AND REQ_SEND_DATE <= '${requestBody.selectDate.end_date} 23:59:59');`;
